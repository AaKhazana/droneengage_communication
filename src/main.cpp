...
{
    std::cout << std::endl
              << _SUCCESS_CONSOLE_BOLD_TEXT_ "Drone-Engage Communicator Server version " << _INFO_CONSOLE_TEXT << version_string << _NORMAL_CONSOLE_TEXT_ << std::endl;
#ifdef DEBUG
...
void _usage(void)
{
    ...
    std::cout << std::endl
              << _INFO_CONSOLE_TEXT "\t--config:          -c ./config.json   default [./de_comm.config.module.json]" << _NORMAL_CONSOLE_TEXT_ << std::ends;
    std::cout << std::endl
              << _INFO_CONSOLE_TEXT "\t--bconfig:         -b ./bconfig.json  default [./de_comm.local]" << _NORMAL_CONSOLE_TEXT_ << std::ends;
    std::cout << std::endl
...
void initModuleManager()
{
    ...
    cUavosModulesManager.defineModule(
        MODULE_CLASS_COMM,
...
void initGPIO()
{
    ...
    // if (!jsonConfig.contains("led_pins") || ((jsonConfig.contains("led_pins_enabled")) && (jsonConfig["led_pins_enabled"].get<bool>() == false)))
    // {
    ...
    // {
        // std::cout << "LEDs disabled as this is not a RPI" << std::endl;
        // led_pins.push_back({pin["name"].get<std::string>(), static_cast<uint8_t>(pin["gpio"].get<int>()), LED_STATUS_OFF});
    // }

    // cLeds.init(led_pins);

    // if (!jsonConfig.contains("buzzer_pins") || ((jsonConfig.contains("buzzer_pins_enabled")) && (jsonConfig["buzzer_pins_enabled"].get<bool>() == false)))
    // {
    ...
    // {
        // std::cout << "Buzzer disabled as this is not a RPI" << std::endl;
...
void init(int argc, char *argv[])
{
    ...
    std::cout << std::endl
              << _SUCCESS_CONSOLE_BOLD_TEXT_ << "=================== " << "STARTING DroneEngage COMMUNICATOR ===================" << _NORMAL_CONSOLE_TEXT_ << std::endl;
...
void quit_handler(int sig)
{
    std::cout << _INFO_CONSOLE_TEXT << std::endl
              << "TERMINATING AT USER REQUEST" << _NORMAL_CONSOLE_TEXT_ << std::endl;
